...
// connect.
func ConnectShadowsocksTunnel(fd int, host string, port int, password, cipher string, isUDPEnabled bool, tk string) (OutlineTunnel, error) {
	...
	t, err := outline.NewTunnel(host, port, password, cipher, isUDPEnabled, tun, tk)
	if err != nil {
		return nil, err
	}
